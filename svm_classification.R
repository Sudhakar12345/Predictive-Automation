...
svm_one = function(data){
  ...
  if(sum(is.na(data1)) > 0 ){
    ...
    svm.model <- svm(dependent ~ ., data = train_data,kernel = "radial", cost = a$cost, gamma = a$gamma)
...
svm_two = function(train, test){
  ...
  if(sum(is.na(train)) > 0 ){
    ...
    svm.model <- svm(dependent ~ ., data = train_dep,kernel = "radial", cost = a$cost, gamma = a$gamma)
