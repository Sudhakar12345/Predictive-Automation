...
decision_one = function(data){
  ...
  if(sum(is.na(data1)) > 0 ){
    ...
    train.fit = rpart(dependent ~ ., method="anova", data = train_data)
    #test_data = test_data[,-c(de)]
    data_predict = predict(train.fit, test_data1, type = "vector")
    ...
    write.csv(test_data,"Predicted_dataset_decision_tree_regression.csv")
    msgBox <- tkmessageBox(title = "Model Run",message = "Successesfully Run the Decision Tree Model for Regression, Please Check your directory to view prediction", icon = "info", type = "ok")
  }
}
...
decision_two = function(train, test){
  ...
  if(sum(is.na(train)) > 0 ){
    ...
    train.fit = rpart(dependent ~ .,method="anova", data = train_dep)
    test_data = test[,c(ind)]
    data_predict = predict(train.fit, test_data, type = "vector")
    data_predict = as.data.frame(data_predict)
    names(data_predict)[1] = "Predicted_data"
    test = cbind(test, data_predict)
    write.csv(test,"Predicted_dataset_decision_tree_regression.csv")
    msgBox <- tkmessageBox(title = "Model Run",message = "Successesfully Run the Decision Tree Model for Regression, Please Check your directory to view prediction", icon = "info", type = "ok")
  }
}
