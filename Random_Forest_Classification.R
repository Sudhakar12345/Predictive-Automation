...
random_one = function(data){
  ...
  if(sum(is.na(data1)) > 0 ){
    msgBox <- tkmessageBox(title = "UnSuccessesful to Run the Random Forest Model",message = "Sorry Please try again..", icon = "info", type = "ok")
  } else {
    names(train_data)[1] = "dependent"
    #set.seed(50)
    library(randomForest)
    train.fit = randomForest(as.factor(dependent) ~ ., data = train_data, importance = TRUE, ntree = 1000)
...
random_two = function(train, test){
  ...
  if(sum(is.na(train)) > 0 ){
    ...
    train.fit = randomForest(as.factor(dependent) ~ ., data = train_dep, importance = TRUE, ntree = 1000)
