...
naiveBayes_one = function(data){
  ...
  if(sum(is.na(data1)) > 0 ){
    ...
    naive_model = naive_bayes(train_data[,-1],as.factor(train_data$dependent),data = train_data,
                              laplace = 1,na.action = stats::na.pass,usekernel = TRUE)
    #plot(naive_model)
    data_predict <- predict(naive_model, test_data1)
    data_predict = as.data.frame(data_predict)
    table(test_data$default, data_predict$data_predict)
    ...
    msgBox <- tkmessageBox(title = "Model Run",message = "Successesfully Run the Naive - Bayes Model, Please Check your directory to view prediction", icon = "info", type = "ok")
  }
}


naiveBayes_two = function(train, test){
  ...
  if(sum(is.na(train)) > 0 ){
    ...
    #train.fit = naiveBayes(dependent ~ ., data = train_dep)
    naive_model = naive_bayes(train_dep[,-1],as.factor(train_dep$dependent),data = train_dep,
                              laplace = 1,na.action = stats::na.pass,usekernel = TRUE)
    #plot(naive_model)
    test_data = test[,c(ind)]
    data_predict <- predict(naive_model, test_data)
