...
svm_one = function(data){
  ...
  if(sum(is.na(data1)) > 0 ){
    msgBox <- tkmessageBox(title = "UnSuccessesful to Run the SVM Regression Model",message = "Sorry Please try again..", icon = "info", type = "ok")
  } else {
    names(train_data)[1] = "dependent"
    #train.fit = glm(dependent ~ .,family=binomial, data = train_data)
    #obj = tune.svm(train_data[,-1],train_data$dependent,cost=10:100,gamma=seq(0,3,0.1))
    #a = obj$best.parameters
    library(e1071)
    svm.model <- svm(dependent ~ ., data = train_data,kernel = "radial",type="nu-regression")
    
    
    #test_data = test_data[,-c(de)]
    data_predict = predict(svm.model, test_data1)
    data_predict = as.data.frame(data_predict)
    names(data_predict)[1] = "Predicted_data"
    test_data = cbind(test_data, data_predict)
    write.csv(test_data,"Predicted_dataset_svm_reg.csv")
    msgBox <- tkmessageBox(title = "Model Run",message = "Successesfully Run the SVM Regression Model, Please Check your directory to view prediction", icon = "info", type = "ok")
    
    }
}


svm_two = function(train, test){
  ...
  if(sum(is.na(train)) > 0 ){
    ...
    #obj = tune.svm(train_dep[,-1],train_dep$dependent,cost=10:100,gamma=seq(0,3,0.1))
    #a = obj$best.parameters
    
    svm.model <- svm(dependent ~ ., data = train_dep,kernel = "radial",type="nu-regression")
    
    test_data = test[,c(ind)]
    data_predict = predict(svm.model, test_data)
    data_predict = as.data.frame(data_predict)
    names(data_predict)[1] = "Predicted_data"
