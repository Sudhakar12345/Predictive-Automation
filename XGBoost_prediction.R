...
predict_xgb_one  = function(data){
  ...
  if(sum(is.na(train_data)) > 0 & sum(is.na(test_data)) > 0){
    ...
    write.csv(test_data,"Predicted_dataset_xgb_reg.csv")
    msgBox <- tkmessageBox(title = "Model Run",message = "Successesfully Run the XGBoost Regression Model, Please Check your directory to view prediction", icon = "info", type = "ok")
  }
}

predict_xgb_two = function(train,test){
  ...
  if(sum(is.na(train_data)) > 0 & sum(is.na(test_data)) > 0){
    ...
    names(data_predict)[1] = "Predicted_data"
    test_data = cbind(test_data, data_predict)
    write.csv(test_data,"Predicted_xgb_pred2.csv")
    msgBox <- tkmessageBox(title = "Model Run",message = "Successesfully Run the XGBoost Regression Model, Please Check your directory to view prediction", icon = "info", type = "ok")
  }
}
...
if(ll == 1){
  predict_xgb_one(data)
} else{
  predict_xgb_two(train,test)
}
source("Home_Page.R")
#predict_xgb_one(data)
#predict_xgb_two(train,test)
